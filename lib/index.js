...
module.exports = new Plugin(
	pkg,
	{symbols: ['REACT_FILE', 'GET_REACT_FILE', 'REACT_ROOT', 'REACT_ROUTER', 'ROUTER_ADD_ROUTE']},
	(Route, {REACT_FILE, GET_REACT_FILE, REACT_ROOT, REACT_ROUTER, ROUTER_ADD_ROUTE}) => {
		...
		return class ReactRoute extends Route {
			...
			async [INIT_ROUTE]() {
				...
				if (!rootRoute) {
					const parentRoute = findParent(route => route[REACT_ROOT]);
				...
				if (!routerRoute && rootRoute !== this) {
					routerRoute = findParent(route => route[REACT_ROUTER]);
					if (routerRoute) this[REACT_ROUTER] = routerRoute;
				}

				// If using router, add route to router
				if (routerRoute) routerRoute[ROUTER_ADD_ROUTE](file, this[URL_PATH]);
			}
